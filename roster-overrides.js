...
// Player roster only. Pre-sign and PM entries are intentionally excluded.
const KFB_ROSTERS = {
  'Yakult Swallows': {
...
function renderKfbRosters() {
  ...
  root.innerHTML = Object.entries(KFB_ROSTERS).map(([team, data]) => {
    const rows = data.players.map((name, i) => `<div class="roster-row"><span>${i + 1}</span><strong>${name}</strong><em></em></div>`).join('');
